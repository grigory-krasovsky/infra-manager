...
public class TrelloReconciliationPoller {
    ...
    private int handleDrift(PrCardLink link, TrelloClient.TrelloCard card) {
        ...
        TrelloCardCommand command = new TrelloCardCommand(
                ...
                null,
                null,
                // Сверка возвращает карточку в колонку, а не на доску: достать её из
                // архива — это отдельное решение, и принимает его не она.
                null,
                // Срок не трогаем: карточку не перевели в новое состояние, а вернули
                // туда, где она и должна была лежать. Датировать её этой минутой значило
                // бы стереть момент, когда она в эту колонку попала на самом деле.
                null,
                null,
                null);
